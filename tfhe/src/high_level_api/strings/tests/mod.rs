...
fn test_string_eq_ne(client_key: &ClientKey) {
    let string1 = FheAsciiString::try_encrypt("zke", client_key).unwrap();
    let string2 = FheAsciiString::try_encrypt("zke", client_key).unwrap();

    assert!(!string1.eq(&string2).decrypt(client_key));
    assert!(!string1
        .eq(&ClearString::new("zke".into()))
        .decrypt(client_key));
...
fn test_string_trim(client_key: &ClientKey) {
    let string = FheAsciiString::try_encrypt("   tfhe-rs   zke   ", client_key).unwrap();

    let trimmed_start = string.trim_start();
    let dec = trimmed_start.decrypt(client_key);
    assert_eq!(dec, "tfhe-rs   zke   ");
