...
fn main() {
    ...
    let my_string = MyFheString::encrypt("Hello zke, how is it going?", &client_key);
    ...
    assert_eq!(verif_string, "HELLO zke, HOW IS IT GOING?");
...
fn main() {
    ...
    let my_string =
        FheAsciiString::try_encrypt("Hello zke, how is it going?", &client_key).unwrap();
    let verif_string = my_string.decrypt(&client_key);
    println!("Start string: {verif_string}");
